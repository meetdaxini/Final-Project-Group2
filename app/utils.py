...
def search_news(query, use_cache=True):
    ...
    articles = [
        article for article in articles_json["articles"] 
        if "https://removed.com" not in article["url"].lower() and "yahoo" not in article["url"].lower()
    ][:5]
    return [newspaper.article(article["url"]) for article in articles]
